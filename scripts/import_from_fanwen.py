...
def get_sfc_connection():
    return pymysql.connect(
        ...
        port=3306, database='sfc_mda',
        user='qinzongqun', password='48HMeJDudJVC8TE',
...
def import_articles(limit=10000):
    print("=" * 60)
    print(f"从 ods_fanwen_article 导入文章 (limit: {limit})")
    print("=" * 60)
    ...
    try:
        ...
        with sfc_conn.cursor() as cursor:
            print("\n正在从 ods_fanwen_article 查询文章...")
            cursor.execute('''
                SELECT articlesequenceid, title, content, pagename, page, typename, literaturetype
                FROM ods_fanwen_article
        ...
        print(f"从 ods_fanwen_article 获取到 {len(articles)} 篇文章")
            ...
            for article in tqdm(articles, desc="导入文章"):
                try:
                    ...
                    ''', (
                        ...
                        article['title'],
                        content,
                        article.get('pagename', ''),
                        article.get('typename', ''),
                        article.get('url', '')
                    ))
